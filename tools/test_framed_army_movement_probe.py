"""Offline native movement producer/guard fixtures; never runs the game or CDB."""
...
import framed_army_movement_probe as probe
...
CAPTURE='C:/ClashCaptures/offline-movement-probe-fixture'
...
class MovementProbeTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        with patch.object(probe.base,'build_selection_probe',side_effect=remember):
            cls.packet=probe.build_movement_probe(cls.original,cls.candidate,cls.save,capture_dir=CAPTURE)
    ...
    def test_source_candidate_save_and_no_side_effects(self):
        ...
        for key in ('candidate','save'):
            ...
            with self.assertRaises(ValueError):probe.build_movement_probe(**args,capture_dir=CAPTURE)
