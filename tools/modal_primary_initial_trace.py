#!/usr/bin/env python3
"""Primary-stage initial-map semantics, copied without weakening native checks.

The inherited source hash records provenance; log and probe are never rewritten.
Standalone results authenticate sequence only, not candidate bytes or readiness.
"""
...
from build_framed_modal_primary_candidate import STAGE
INHERITED_SEMANTICS_SHA256 = "34a022251bfd572b5f6ecfbedd9a1ccd23a9ded1ba749039201c133108528b0a"
INTEGRITY_SHA256 = "a20512fa49cc86db67a486f9202d4efa3f9f3745005d11220bf6097661a44729"
...
def evaluate_trace(log: str, probe: str, *, resolution: str, candidate_sha256: str, stage: str) -> dict:
    integrity_source = Path(__file__).with_name('partial_tile_trace_probe.py').read_bytes()
    if hashlib.sha256(integrity_source).hexdigest() != INTEGRITY_SHA256:
        raise ValueError('initial event-integrity helper source changed')
    ...
    # This new lane preserves the authentic PTILE stage. Source/candidate
    # authentication belongs to the full primary trace, never a stage alias.
    contract_stage = stage
    framed = True
    if stage != STAGE:
        failures.append('exact owned-modal-primary stage required')
