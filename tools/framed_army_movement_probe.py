#!/usr/bin/env python3
"""Pure, source-bound preview/confirmation of one native whole-army move.

The existing selection-v3 protocol is copied under WMOV_BASE. A new diagnostic
...
ROOT = base.ROOT
BASE_SHA256 = '53b39c60267fd68a697d47aefac0fdf5acfb1c9906c764c36ea8e0d0ea33b8ef'
REVISION = 'controlled_native_whole_army_outward_move_v3'
UNIT_OFFSET = 149349
...
def build_movement_probe(original, candidate, save, *, capture_dir, minimap_viewport=True):
    ...
    for index,name in enumerate(('movement-before','movement-preview','movement-after')):
        ...
        action+=f'; .writemem {directory}/{name}.header.raw poi(005202e0) L0n188; .writemem {directory}/{name}.raw poi(poi(005202e0)+4) L0n786432; .writemem {directory}/{name}.unit.raw @$t1+0n149349 L0n725; '
    ...
    verify_sources()
    return dict(schema='clash95_framed_army_movement_probe_v1',revision=REVISION,stage=prior['stage'],resolution='1024x768',
        ...
        capture_dir=directory,minimap_viewport=minimap_viewport,unit=3,source_xy=[16,19],destination_xy=[18,19],
