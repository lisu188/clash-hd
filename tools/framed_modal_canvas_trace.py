...
def compile_probe(packet: dict) -> str:
    ...
    geometry = render_probe(BASE_PROBE.read_text(encoding="utf-8-sig"), packet["resolution"], producer.builder.BASE_STAGE)["geometry"]
    replacements = {
...
def evaluate_sequence(log: str, packet: dict) -> dict:
    ...
    if route is None or packet.get("stage") != producer.STAGE or packet.get('protocol_revision') != producer.PROTOCOL_REVISION:
    ...
    for number, text in enumerate(log.splitlines(), 1):
        if re.search(r"AV_SURFDUMP|SURFDUMP_INVALID|PTILE_REJECT|MCAP_REJECT|MCANVAS_CONTRACT_FAIL|\bMODAL_|syntax error|couldn't resolve", text, re.I):
    ...
    loaded_contract=(f"MCANVAS_CONTRACT_PASS stage={producer.STAGE} resolution={packet['resolution']} "
                     f"candidate_sha256={packet.get('candidate_sha256')} revision={producer.builder.REVISION} "
                     f"state={state_va:08x} state_bytes=4096")
    loaded_scope="MCANVAS_SCOPE owned_castle_canvas_only manual_input_proof=false promotion_ready=false"
    canvas_startup=[(n,t) for n,t in enumerate(log.splitlines(),1) if re.search(r'\bMCANVAS_',t,re.I)]
    if [t for _,t in canvas_startup] != [loaded_contract,loaded_scope]:
        fail('owned-canvas loaded contract/scope missing, repeated, malformed or mismatched')
    elif not rows or canvas_startup[-1][0]>=next((r['line'] for r in rows if r['marker']=='MCAP_CONTRACT'),0):
        fail('owned-canvas loaded contract must precede modal startup contract')
    elif next((r['line'] for r in rows if r['marker']=='MCAP_BYTE_CONTRACT_PASS'),10**9)>=canvas_startup[0][0]:
        fail('native-route byte contract must precede owned-canvas loaded contract')
...
def _project_initial(log, extra, packet):
    ...
    projected=contract.replace(f'stage={producer.STAGE} ',f'stage={producer.builder.BASE_STAGE} ',1)
    outputs=[];binding={'projection':'one_exact_initial_contract_stage_token',
                        'source_stage':producer.STAGE,'projected_stage':producer.builder.BASE_STAGE}
...
def evaluate_trace(log: str, *, original: bytes, candidate: bytes, packet: dict, generated_probe: bytes) -> dict:
    ...
    passed=not failures
    return dict(schema='clash95_framed_modal_canvas_trace_v1',passed=passed,ready_for_host_capture=passed,
        ...
        initial_map_projection=projection,
