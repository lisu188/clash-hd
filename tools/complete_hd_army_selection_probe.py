#!/usr/bin/env python3
"""Prepare controlled native army selection on the exact complete-HD candidate.

This is a new protocol source revision derived from frozen selection-v3. It
reconstructs the complete manifest, candidate and all inherited loaded-byte
contracts for each supported resolution. Startup uses an explicit immutable
recipe rather than importing a changing runtime harness. No process, input,
capture, output file, acceptance or promotion is performed here.
"""
...
PARENT_SOURCE_SHA256 = "52b517b641ca2e81d1892e71ab4c3c60ab722a05d97c2419dd2cc666349773a1"
PINNED_SOURCES = {
...
SOURCE_PATHS = (
    "tools/complete_hd_army_selection_probe.py", "tools/army_probe_startup.py",
    "tools/complete_hd_runtime_context.py", "src/patcher/complete_hd_candidate.py",
)
...
LIMITS = [
    ...
    "The original four-update trace must be separately validated with exact complete candidate context; all ARMY/COMPLETEHD/PTILE contracts remain required.",
...
def canonical_json(value):
    """Serialize exact JSON values; reject aliases, cycles and excessive depth."""
    active = set()
    ...
    def check(item):
        if type(item) in (dict, list):
            ...
            raise ValueError("only exact JSON value types are accepted")
...
def verify_sources():
    ...
    return dict(PINNED_SOURCES) | {name: sha((ROOT / name).read_bytes()) for name in SOURCE_PATHS}
...
def build_selection_probe(original: bytes, candidate: bytes, save_bytes: bytes, *,
                          capture_dir: str, candidate_manifest: dict, resolution: str) -> dict:
    """Return a deterministic packet, command text and initial observer text.

    Reconstructs the full complete candidate and checks every supplied manifest
    field against its deterministic rebuild. No probe overrides are accepted. The caller
    ...
    if type(candidate_manifest) is not dict:
        raise ValueError("complete candidate manifest must be an object")
    canonical_manifest = canonical_json(candidate_manifest)
    if resolution not in builder.RESOLUTIONS:
        raise ValueError("unsupported complete-HD resolution")
    ...
    context = runtime.verify_context(candidate_manifest, original, resolution=resolution, candidate=candidate)
    metadata, initial_extra = context["manifest"], context["probe"]
    if canonical_manifest != json.dumps(metadata, sort_keys=True, separators=(",", ":"), allow_nan=False):
        raise ValueError("complete candidate manifest value types differ from reconstruction")
    inherited = metadata["predecessor"]
    ...
    entries = inherited["army_entry_vas"]
    ...
    surface = (f"(wo(poi(005202e0)) == 0n{width}) & (wo(poi(005202e0)+2) == 0n{height}) & "
               f"(poi(poi(005202e0)+4) >= 00010000) & (poi(poi(005202e0)+4) <= {0x100000000-width*height:08x})")
    records = {}
    ...
    for number, va, phase, name in ((88, before, 7, "before-redraw"), (89, after, 8, "after-redraw")):
        ...
        body += f"; .writemem {directory}/{name}.raw poi(poi(005202e0)+4) L0n{width*height}; gc"
    ...
    template = render_probe(BASE_PROBE.read_text(encoding="utf-8-sig"), resolution, compiler.BASE_STAGE)["template"]
    ...
    if sources != verify_sources():
        raise ValueError("army selection sources changed during preparation")
    return dict(schema="clash95_complete_hd_army_selection_probe_v1", revision=REVISION, stage=builder.STAGE,
        resolution=resolution, width=width, height=height, candidate_sha256=sha(candidate), original_sha256=sha(original),
        ...
        capture_dir=directory, minimap_viewport=True, compiled_probe=compiled, probe_sha256=sha(compiled.encode("ascii")),
        ...
        candidate_manifest_canonical_sha256=sha(canonical_manifest.encode()),
        candidate_recipe=metadata["recipe_revision"], inherited_stage=inherited["stage"],
        inherited_revision=inherited["army_revision"], startup_recipe=compiler.provenance(),
        parent_source_sha256=PARENT_SOURCE_SHA256,
        source_sha256=metadata["source_hashes"] | sources, manual_input_proof=False, promotion_ready=False,
...
def _unique_object(pairs):
    ...
    for key, value in pairs:
        if key in result:
            raise ValueError("duplicate candidate manifest key: " + key)
        result[key] = value
    return result
...
def main():
    ...
    manifest = read_manifest(args.candidate_manifest)
    packet = build_selection_probe(args.original.read_bytes(), args.candidate.read_bytes(),
        args.save.read_bytes(), capture_dir=args.capture_dir,
        candidate_manifest=manifest, resolution=args.resolution)
