...
def classify(report: dict[str, Any]) -> tuple[str, str, bool]:
    ...
    if "route/input probe failures" in failures:
        return (
            "input_route_failure",
    ...
    if "frame sample count" in failures or int(report.get("frame_sample_count") or 0) == 0:
        return (
...
def build_triage(report: dict[str, Any], source_report: Path | None = None) -> dict[str, Any]:
    classification, next_probe, passed = classify(report)
...
def to_markdown(triage: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "## Last Evidence",
        "",
        f"- Last route: `{route.get('name')}` path=`{route.get('path_verified')}` click=`{route.get('click_path_verified')}`",
        (
            "- Last frame: "
    ...
    for key in ("process_sample_count", "working_set_growth_bytes", "handle_growth", "artifact_bytes"):
