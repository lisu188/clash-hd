#!/usr/bin/env python3
"""Prepare a separate owned-canvas castle probe; never run a debugger.
...
import framed_screen_probe as legacy
...
BUILDER_SHA256 = "ec7c933fd9112b556636510f0b762076fc5f8c4c2e86bbc704b70f26cddf98d1"
...
def _capture_canvas(route, width, height, metadata):
    ...
    body = _canvas_record(metadata, "READY") + "; bd 91; bd 93; bd 98; " + capture
...
def canonical_template(original: bytes, resolution: str, *, minimap_viewport: bool = False):
    if sha(Path(builder.__file__).read_bytes()) != BUILDER_SHA256:
        raise ValueError("reviewed whole modal-canvas builder source changed")
    ...
    template = render_probe(base, resolution, builder.BASE_STAGE)["template"]
...
def build_screen_probe(original: bytes, candidate: bytes, *, candidate_sha256: str,
    ...
    profile = legacy.builder.recipe.patcher.parse_resolution(resolution)
    ...
    return dict(schema="clash95_framed_modal_canvas_probe_packet_v1", prepared=True, runtime_ready=False,
        ...
        canvas_revision=builder.REVISION, builder_sha256=sha(Path(builder.__file__).read_bytes()),
        legacy_producer_sha256=sha(Path(legacy.__file__).read_bytes()),
