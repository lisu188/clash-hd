"""Reconstruct the exact text-v1 bundle without running or accepting a game.
...
SCHEMA = 'clash95_framed_modal_primary_text_candidate_v1'
...
def _source_paths(sources) -> dict[str, Path]:
    ...
    for name, expected in FROZEN_TEXT_SOURCES.items():
        if sources.get(name) != expected:
            raise ValueError('frozen text source identity differs: ' + name)
...
def _identity(manifest, original, candidate, probe):
    if (manifest.get('schema') != SCHEMA or manifest.get('stage') != STAGE
        ...
        raise ValueError('exact text-v1 manifest schema/stage/revision required')
    if type(manifest.get('resolution')) is not str or manifest['resolution'] not in RESOLUTIONS:
        raise ValueError('exact text-v1 fixture resolution required')
    if manifest.get('base_sha256') != sha(original) or manifest.get('original_sha256') != sha(original):
    ...
    primary = manifest.get('base_candidate')
    ...
    for child, suffix, revision in (
        ...
        expected_stage = STAGE.removesuffix('-modalprimarytext-validation') + suffix
        if (type(child) is not dict or child.get('stage') != expected_stage
            ...
            raise ValueError('text candidate predecessor chain differs')
        parent = child
    try:
        ...
        if (type(owner) is not dict or owner['state_va'] != manifest['modal_state_va']
            ...
            raise ValueError('text inherited owner context differs')
    except (KeyError, TypeError) as error:
        raise ValueError('text inherited owner context missing') from error
    return primary, slots, owner
...
def load_context(original: bytes, candidate: bytes, manifest_path: Path) -> dict:
    ...
    if not path.name.endswith('.candidate.json'):
        raise ValueError('text manifest must have .candidate.json suffix')
    stem = path.name.removesuffix('.candidate.json')
    if not stem:
        raise ValueError('text bundle requires a named candidate')
    ...
    if type(image) is not bytes or type(probe) is not str or image != candidate or probe.encode('utf-8') != probe_raw:
        raise ValueError('candidate/probe differ from exact frozen text-source rebuild')
    ...
    primary, slots, owner = _identity(manifest, original, candidate, probe_raw)
    if _source_snapshots(sources, source_paths) != before or _snapshot(Path(__file__)) != reader:
    ...
    return dict(
        schema='clash95_modal_primary_text_context_v1', stage=STAGE, recipe_revision=REVISION,
        ...
        source_hashes=dict(sources), context_source_sha256=sha(reader.data),
