...
import modal_primary_text_context as context


class TextContextTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='mptctx-')
        ...
        self.candidate = b'synthetic text candidate bytes'
        self.probe = '.echo synthetic text-context fixture\n'
    ...
    def make_manifest(self):
        prefix = context.STAGE.removesuffix('-modalprimarytext-validation')
        ...
        manifest = child(primary, '-modalprimarytext-validation', context.REVISION, context.sha(self.candidate))
        manifest.update(schema=context.SCHEMA, base_sha256=context.sha(self.original),
    ...
    def test_exact_bundle_returns_distinct_real_ancestor_contexts(self):
        ...
        self.assertNotEqual(result['primary_context']['stage'], result['stage'])
        ...
        result['primary_context']['stage'] = 'changed'
        self.assertNotEqual(result['manifest']['base_candidate']['stage'], 'changed')
    ...
    def test_mutated_ancestor_cannot_replace_text_context(self):
    ...
    def test_owner_state_or_entries_mismatch_rejected(self):
        for key, value in [('state_va', 0x602000), ('modal_entry_vas', {'is_active': 0x600010})]:
            mutated = copy.deepcopy(self.manifest)
            owner = mutated['base_candidate']['base_candidate']['base_candidate']['predecessor']['base_candidate']
            owner[key] = value
            self.write_bundle(mutated)
    ...
    def test_mismatched_or_missing_siblings_rejected(self):
        for target in (self.exe, self.command):
    ...
    def test_source_paths_and_pins_fail_before_source_reads(self):
        ...
        for change in ('wrong', 'missing'):
            mutated = copy.deepcopy(self.manifest); key = next(iter(context.FROZEN_TEXT_SOURCES))
            if change == 'wrong': mutated['source_hashes'][key] = '0' * 64
            else: del mutated['source_hashes'][key]
            self.write_bundle(mutated)
            with self.assertRaisesRegex(ValueError, 'frozen text source identity|recipe source path set'): self.load()
        self.build.assert_not_called()

    def test_missing_or_extra_recipe_source_rejected_before_resolution(self):
        # The entire 34-source inventory is admitted before even resolving a
        ...
        cases.extend([extra, dict(context.FROZEN_TEXT_SOURCES)])
    ...
    def test_source_byte_mismatch_rejected_before_builder(self):
        key = next(iter(context.FROZEN_TEXT_SOURCES)); path = self.repo / key
        path.write_bytes(path.read_bytes() + b'\n# changed\n')
        with self.assertRaisesRegex(ValueError, 'source hash differs'): self.load()
        self.build.assert_not_called()

    def test_hardlinked_source_alias_rejected(self):
        key, alias = sorted(context.RECIPE_SOURCE_PATHS - context.FROZEN_TEXT_SOURCES.keys())[:2]
    ...
    def test_drift_during_rebuild_rejected(self):
        source = self.repo / next(iter(context.FROZEN_TEXT_SOURCES))
    ...
    def test_rebuild_candidate_probe_and_source_failures_propagate(self):
        for result in [(b'wrong', self.manifest, self.probe), (self.candidate, self.manifest, 'wrong')]:
            self.build.return_value = result
            with self.assertRaisesRegex(ValueError, 'frozen text-source rebuild'): self.load()
