...
- Overall: PASS
- Generated: `2026-09-06T05:57:49+02:00`
...
- All commands carry the additive guest lane: `True`
- All guest commands require guest approval: `True`
- Guest transport tool: `tools\vm_guest_click.py` (class `approved_guest_win98_directdraw`, machine `clash-hd-vm`)
...
- Guest lane transport: `tools\vm_guest_click.py` -> guest `clash-hd-vm` (class `approved_guest_win98_directdraw`, approval-gated)

```powershell
python 'tools\vm_guest_click.py' --target 'stable_menu_load' --guest-machine 'clash-hd-vm' --qmp-port 4445 --candidate 'C:\ClashTests\manual-directinput\<stable-hd-map-candidate-exe>' --evidence-class 'approved_guest_win98_directdraw' --pulse-route-steps 'load-button:302,211;load-slot0:320,166;confirm-load:400,226' --allow-guest-input
```
...
```powershell
python 'tools\vm_guest_click.py' --target 'stable_hd_map_input' --guest-machine 'clash-hd-vm' --qmp-port 4445 --candidate 'C:\ClashTests\manual-directinput\<stable-hd-map-candidate-exe>' --evidence-class 'approved_guest_win98_directdraw' --pulse-route-steps 'load-button:302,211;load-slot0:320,166;confirm-load:400,226' --followup-points 'map-center:400,300;map-right-edge:780,300;map-bottom:400,580;minimap-right-bottom:760,560' --allow-guest-input
```
...
```powershell
python 'tools\vm_guest_click.py' --target 'right_bottom_validation_input' --guest-machine 'clash-hd-vm' --qmp-port 4445 --candidate 'C:\ClashTests\manual-directinput\<rightbottomcompose-candidate-exe>' --evidence-class 'approved_guest_win98_directdraw' --pulse-route-steps 'load-button:302,211;load-slot0:320,166;confirm-load:400,226' --followup-points 'action-ui:588,440;grid-hit:450,73;minimap-right-bottom:760,560' --allow-guest-input
```
...
```powershell
python 'tools\vm_guest_click.py' --target 'castle_barracks_centered_input' --guest-machine 'clash-hd-vm' --qmp-port 4445 --candidate 'C:\ClashTests\manual-directinput\<castlecenter-all-candidate-exe>' --evidence-class 'approved_guest_win98_directdraw' --pulse-route-steps 'load-button:302,211;load-slot0:320,166;confirm-load:400,226' --followup-points 'castle-entry:470,397;castle-0x63:231,366;barracks-0x86:398,228' --allow-guest-input
```
...
```powershell
python 'tools\vm_guest_click.py' --target 'castle_overview_centered_input' --guest-machine 'clash-hd-vm' --qmp-port 4445 --candidate 'C:\ClashTests\manual-directinput\<castlecenter-all-candidate-exe>' --evidence-class 'approved_guest_win98_directdraw' --pulse-route-steps 'load-button:302,211;load-slot0:320,166;confirm-load:400,226' --followup-points 'castle-entry:470,397;castle-0x63:231,366;owner-action:180,440;overview-0x87:503,426' --allow-guest-input
```
...
- the additive guest lane transports the same route/follow-up coordinates through vm_guest_click.py into the clash-hd-vm Win98 guest (QMP 4445); it stays a template until explicit approval, requires --allow-guest-input, and records the DISTINCT approved_guest_win98_directdraw class - never silently as host manual_directinput evidence
