...
- Overall: PASS
- Generated: `2026-07-18T21:30:54+02:00`
...
- `save_load_roundtrip`: status=`pass` passed=`True` - continuity proof passes
- `turn_advancement`: status=`pass` passed=`True` - continuity proof passes
- `campaign_routes`: status=`pass` passed=`True` - continuity proof passes
