...
function New-CanvasCapturePlan {
    ...
    $trace = Resolve-CanvasPath (Join-Path $script:RepoRoot 'tools\framed_modal_canvas_trace.py') -Kind file
    $producer = Resolve-CanvasPath (Join-Path $script:RepoRoot 'tools\framed_modal_canvas_probe.py') -Kind file
    ...
    $plan = [ordered]@{
        schema='clash95_framed_modal_canvas_capture_plan_v1'; environment='hidden_cdb_host'; execute=[bool]$Options.Execute
        ...
        limits=@('Controlled native dispatch and optional availability mutation; no natural-input proof.',
            'One paused physical HD mirror snapshot; E0 remains the real native640 canvas. Primary-only overlays may be absent.',
...
function Invoke-CanvasCapture {
    ...
    $summary = [ordered]@{ schema='clash95_framed_modal_canvas_capture_v1'; passed=$false; status='failed'; executed=$false; plan=$plan
...
try {
    ...
    @{ schema='clash95_framed_modal_canvas_capture_v1'; passed=$false; status='preparation_failed'; executed=$false
