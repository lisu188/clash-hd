<# Additive hidden primary capture using the reviewed owned-modal-canvas lifecycle. Default is an offline plan only.
...
function Invoke-CanvasPythonJson {
    ...
    try {
        ...
        if ($exitCode -ne 0 -and -not $PermitFailure) {throw "Offline Python command failed with exit $exitCode. $errorText"}
...
function New-CanvasCapturePlan {
    ...
    $trace = Resolve-CanvasPath (Join-Path $script:RepoRoot 'tools\framed_modal_primary_capture.py') -Kind file
    $producer = Resolve-CanvasPath (Join-Path $script:RepoRoot 'tools\framed_modal_primary_capture.py') -Kind file
    ...
    if ($sourceHash -cne '407d41d0d548e6217c6041181efb6466efa90f447110c5b8857bf92714ecb8eb' -or
        $proxyHash -cne 'b173a9dd4ce772eb5b56f341acdf4b329ed4ffdd638c1fce5cc37dd332804b70') {throw 'Primary capture requires the exact reviewed memory proxy before launch.'}
    ...
    $prepareArgs = @($trace,'--prepare','--original',$originalPath,'--candidate',$inputPath,
        ...
        '--ready-file',(Join-Path $outputDirectory 'primary-ready.cdb'))
    ...
    $plan = [ordered]@{
        schema='clash95_framed_modal_primary_capture_plan_v1'; environment='hidden_cdb_host'; execute=[bool]$Options.Execute
        ...
        primary_snapshot_required=$true; manual_input_proof=$false; visible_composition_proof=$false; promotion_ready=$false
        limits=@('Controlled native dispatch and optional availability mutation; no natural-input proof.',
            'Separate paused physical mirror, native640 and actual proxy-primary buffers; current attached palette is read with the primary.',
            'A successful source-bound full primary Lock does not establish every intended UI layer or its pixel correctness.',
...
function Assert-CanvasSurface {
    param($Report, $Plan)
    if ($Report.primary_sequence.passed -isnot [bool] -or -not $Report.primary_sequence.passed -or $Report.passed -isnot [bool] -or -not $Report.passed -or
        $Report.ready_for_host_capture -isnot [bool] -or -not $Report.ready_for_host_capture -or
        $Report.schema -cne 'clash95_framed_modal_canvas_trace_v1' -or $Report.stage -cne $Plan.stage -or
...
function Find-CanvasOwnedChildren {
    ...
    foreach ($row in $matches) {
        ...
        # Add PROCESS_QUERY_INFORMATION (0x400) for VirtualQueryEx. Retain
        # the original exact-child, read, synchronization and cleanup rights.
...
function Save-PrimarySnapshot {
    ...
    return @{schema='clash95_modal_primary_snapshot_receipt_v1';run_id=[IO.Path]::GetFileName($Plan.out_dir);
...
function Invoke-CanvasCapture {
    ...
    $summary = [ordered]@{ schema='clash95_framed_modal_primary_capture_v1'; passed=$false; status='failed'; executed=$false; plan=$plan
    ...
    $logPath=Join-Path $plan.out_dir 'cdb.log'; $probePath=Join-Path $plan.out_dir 'framed-modal-canvas.cdb'
    ...
    try {
        ...
        if ((Get-CanvasHash $plan.primary_ready_path) -cne $plan.primary_ready_sha256) {throw 'Primary readiness file changed before launch.'}
        ...
        $verifyArgs += @('--ready-file',$plan.primary_ready_path)
        ...
        while ($watch.Elapsed.TotalSeconds -lt 120) {
            ...
            if ($commandFailure) {
                $failureBytes=Read-CanvasLogBytes $logPath
                $failurePath=Join-Path $plan.out_dir 'command-failure-prefix.log'
                [IO.File]::WriteAllBytes($failurePath,$failureBytes)
                $commandFailure.log_prefix=@{path=$failurePath;bytes=$failureBytes.Length;sha256=(Get-CanvasHash $failurePath)}
                $summary.debugger_command_failure=$commandFailure
                throw ('Debugger command failure (not a game crash): '+$commandFailure.text)
            }
            if (Test-CanvasModalReady $text) {
                $summary.trace=Invoke-CanvasPythonJson $plan.python @($plan.trace,'--log',$logPath,'--packet',$packetPath,
                    '--original',$plan.original,'--candidate',$plan.candidate_path,'--probe',$probePath) -PermitFailure -TimeoutMilliseconds ([Math]::Max(1,120000-[int]$watch.ElapsedMilliseconds))
                ...
                $summary.snapshot=Save-CanvasSnapshot @($owned.Values)[0] $surface $rawPath $plan
                $summary.primary_snapshot=Save-PrimarySnapshot @($owned.Values)[0] $summary.trace $plan $summary.capture_prefix
                $primaryManifest=Join-Path $plan.out_dir 'primary-snapshot.json'; Write-CanvasJson $primaryManifest $summary.primary_snapshot
        ...
        if ($summary.primary_snapshot) {
            try {
                $r=$summary.primary_snapshot
                # Audit only after exact process cleanup succeeds. A cleanup
                # failure retains raw/PNG but must not prolong a live target.
                if ($summary.cleanup.cdb.absent -and $summary.cleanup.cdb.handle_closed -and
                    ...
                    $summary.primary_audit=Invoke-CanvasPythonJson $plan.python @($plan.trace,'--log',$summary.capture_prefix.path,'--packet',$packetPath,'--probe',$probePath,
                        '--original',$plan.original,'--candidate',$plan.candidate_path,'--snapshot-manifest',$primaryManifest,'--proxy',$plan.proxy_path) -PermitFailure
                    Write-CanvasJson (Join-Path $plan.out_dir 'primary-audit.json') $summary.primary_audit
                    if (-not $summary.primary_audit.passed -or -not $summary.primary_audit.primary_snapshot.cached_primary_snapshot_valid) {$summary.failures+='Primary snapshot failed source-bound validation.'}
                }
                if ((Get-CanvasHash $r.pixels.path) -cne $r.pixels.sha256 -or (Get-CanvasHash $r.palette_entries.path) -cne $r.palette_entries.sha256) {throw 'Paused primary bytes or attached palette changed before PNG conversion.'}
                $primaryPng=Join-Path $plan.out_dir 'primary.png'; $primaryMeta=Join-Path $plan.out_dir 'primary-png.json'
                $convertOutput = & $plan.python -B $plan.converter $r.pixels.path --width $plan.width --height $plan.height --pitch $plan.width --output $primaryPng --metadata $primaryMeta --log $summary.capture_prefix.path --palette $r.palette_entries.path
                if ($LASTEXITCODE -ne 0) {throw 'Current primary PNG conversion failed.'}
                $summary.primary_png=[IO.File]::ReadAllText($primaryMeta) | ConvertFrom-Json
                if ($summary.primary_png.palette_mode -cne 'directdraw-palette' -or $summary.primary_png.raw_sha256 -cne $r.pixels.sha256 -or
                    (Get-CanvasHash $r.palette_entries.path) -cne $r.palette_entries.sha256 -or $summary.primary_png.png_sha256 -cne (Get-CanvasHash $primaryPng)) {throw 'Primary PNG binding failed.'}
            } catch {$summary.failures+=$_.Exception.Message}
        }
        if ($summary.executed) {
            try {
        ...
        $summary.passed=($summary.primary_audit.passed -and $summary.primary_audit.primary_snapshot.cached_primary_snapshot_valid -and $null -ne $summary.primary_png -and $summary.executed -and $summary.failures.Count -eq 0 -and $null -ne $summary.snapshot -and $null -ne $summary.png)
...
try {
    ...
    @{ schema='clash95_framed_modal_primary_capture_v1'; passed=$false; status='preparation_failed'; executed=$false
