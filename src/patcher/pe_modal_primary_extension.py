"""Additive thirteenth RX section above the exact slots-format predecessor.

The only production caller is the new primary builder, which reconstructs the
slots candidate and obtains its source-bound emitter before using this private
...
def allocation_layout(candidate: bytes) -> AllocationLayout:
    ...
    pe._require(len(image.sections) == 12 and tuple(s.name.rstrip(b'\0') for s in image.sections[-5:])
        == (b'.hdcode', b'.hdmodal', b'.hdstate', b'.hdarmy', b'.hdslots'),
        'exact twelve-section slots layout required')
    pe._require(all(image.sections[index].characteristics == pe.RX_CODE for index in (-5,-4,-2,-1))
        and image.sections[-3].characteristics == 0xC0000040, 'slots section protections differ')
    ...
    pe._require(all(a.rva + pe._align(a.memory_size,4096) == b.rva for a,b in zip(image.sections,image.sections[1:])),
        'slots sections have nonadjacent extents')
    slot = image.sections[-1].header_offset + 40
    pe._require(image.sections[-1].header_offset == 0x320 and slot == 0x348
...
def _extend_verified_image(candidate, *, code, code_va, relocations, hooks, binding):
    ...
    Five declared equal-length hook replacements are required. The placeholder
    may occupy ten bytes; hook length is never truncated to a five-byte branch.
    ...
    pe._require(isinstance(relocations,(tuple,list)) and isinstance(hooks,(tuple,list)) and len(hooks) == 5,
        'five explicit primary hooks and declared relocations required')
    ...
    header = struct.pack('<8sIIIIIIHHI',b'.hdprim',CODE_RESERVATION,layout.code_rva,raw_size,len(candidate),0,0,0,0,pe.RX_CODE)
    ...
    change(before.pe_offset+6,struct.pack('<H',13),'NumberOfSections')
    ...
    change(layout.code_header_offset,header,'.hdprim RX header')
    ...
    pe._require(after.sections[:12]==before.sections,'historical section header changed')
    ...
    metadata.update(schema='clash95_pe_modal_primary_extension_v1',input_sha256=pe._sha(candidate),
